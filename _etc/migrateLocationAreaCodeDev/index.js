		...
		for (const item of items) {
			...
			try {
				if (item.location.origin && !item.location.origin.areaCode) {
					...
					await MONGO_DB.update(ENV.MONGO_DB.DEV.ID, targetCollection, { _id: ObjectId(item._id.toString()) }, { $set: { "location.origin.areaCode": areaCode } });
				}
			} catch (error) {}
		}
	} catch (error) {
...
async function getAreaCode(locationInfo) {
	try {
		let result = null;
		// 주소 1,2,3(h)단계 확인
		result = await MONGO_DB.findOne(ENV.MONGO_DB.DEV.ID, "weather", { region_1depth: locationInfo.region_1depth, region_2depth: locationInfo.region_2depth, region_3depth: locationInfo.region_3depth_h });
		if (result && result.areaCode) {
			return result.areaCode;
		}

		// 주소 1,2,3단계 확인
		result = await MONGO_DB.findOne(ENV.MONGO_DB.DEV.ID, "weather", { region_1depth: locationInfo.region_1depth, region_2depth: locationInfo.region_2depth, region_3depth: locationInfo.region_3depth });
		if (result && result.areaCode) {
			return result.areaCode;
		}

		// 주소 2,3(h)단계 확인
		result = await MONGO_DB.findOne(ENV.MONGO_DB.DEV.ID, "weather", { region_2depth: locationInfo.region_2depth, region_3depth: locationInfo.region_3depth_h });
		if (result && result.areaCode) {
			return result.areaCode;
		}

		// 주소 2,3단계 확인
		result = await MONGO_DB.findOne(ENV.MONGO_DB.DEV.ID, "weather", { region_2depth: locationInfo.region_2depth, region_3depth: locationInfo.region_3depth });
		if (result && result.areaCode) {
			return result.areaCode;
		}

		// 주소 1,2단계 확인
		result = await MONGO_DB.findOne(ENV.MONGO_DB.DEV.ID, "weather", { region_1depth: locationInfo.region_1depth, region_2depth: locationInfo.region_2depth });
		if (result && result.areaCode) {
			return result.areaCode;
		}

		// 주소 1단계 확인
		result = await MONGO_DB.findOne(ENV.MONGO_DB.DEV.ID, "weather", { region_1depth: locationInfo.region_1depth });
		if (result && result.areaCode) {
			return result.areaCode;
		}
	} catch (error) {
