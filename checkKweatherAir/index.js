...
async function main() {
	await initMongoDb();
	await start("./AIR_POLLUTION_20201120");
}
...
async function handleJson(es, data) {
	...
	let area = {
		...
		PM10_VAL: data.PM10_VAL,
		PM25_VAL: data.PM25_VAL,
	};
	//if (data.PM10_VAL === "-" || data.PM25_VAL === "-") {
	// console.log("AREA_CODE : " + area.areaCode + ", " + area.region_1depth + " " + area.region_2depth + " " + area.region_3depth + ", PM10_VAL : " + area.PM10_VAL + ", PM25_VAL : " + area.PM25_VAL);
	console.log(area.areaCode + ", " + area.region_1depth + " " + area.region_2depth + " " + area.region_3depth + ", " + area.PM10_VAL + ", " + area.PM25_VAL);
	//}
	try {
		//await SLEEP(10);
		es.resume();
...
function convertAreaNameWide(areaNameWide) {
	if (areaNameWideList[areaNameWide]) {
		...
		// console.log("from : " + areaNameWide + ", to : " + areaNameWide);
		areaNameWide;
	}
}
